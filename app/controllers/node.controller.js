...
exports.findAll = function(req, res){
  ...
  Note.find(function(err, notes){
    if(err) {
      res.status(500).send({message: "Some error occured while retrieving the notes"});
    } else {
      res.send(notes);
    }
...
exports.findOne = function(req, res){
  //Find a single node with a noteId
...
exports.update = function(req, res){
  ...
  note.save(function(err, data){
    if(err) {
      res.status(500).send({message: "Could not update a note with id " + req.params.noteId});
    } else {
      res.send(data);
    }
  });
